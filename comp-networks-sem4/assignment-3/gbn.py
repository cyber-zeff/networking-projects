...
class GBNSender:
    ...
    def send_all(self, data_list):
        ...
        print(f"\n[GBN SENDER] starting.. win size: {self.win_size}, total pkts: {total}")
        ...
        while self.base < total and retries <= MAX_RETRIES:
            ...
            while self._win_available() > 0 and next_data_i < total:
                ...
                self.win[seq] = pkt
                self.channel.transmit(copy.copy(pkt), log_prefix="CHANNEL -> RX")
                self.next_seq += 1
                next_data_i += 1
            ...
            for i in range(self.base, next_data_i):
                ...
                ack = gbn_receiver_fsm(
                    self.channel.transmit(copy.copy(pkt), log_prefix=f"RETRY -> RX seq: {seq}"),
            ...
            for ack in acks_this_round:
                ...
                while self.base <= ack_n and self.base < total:
                    ...
                    if confirmed_seq in self.win:
                        print(f"[GBN SENDER] PKT seq={confirmed_seq} ACKed.")
...
def gbn_receiver_fsm(packet, log_prefix=""):
    ...
    print(f"[GBN RECEIVER {log_prefix}] OK seq: {packet.seq_num}, data: '{packet.data}'")
    ...
    print(f"[GBN RECEIVER {log_prefix}] sending ACK({packet.seq_num})")
    return ack


def run_gbn(pkt_count=6, pkt_size=10, win_size=4, loss_prob=0.2, corrupt_prob=0.2, delay_prob=0.1):
    print("=" * 65)
    print("GBN -- Go-Back-N Protocol Simulation")
    print("=" * 65)
    ...
    print(f"Corrupt probability: {corrupt_prob}")
    print(f"Delay probability: {delay_prob}")
    print("=" * 65)
